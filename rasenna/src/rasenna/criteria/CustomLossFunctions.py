...
from .TopologicalLossFunction import TopologicalLossFunction
from .SubdivisionLossFunction import SubdivisionLossFunction
...
class TopologicalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        if not self.pretraining:
            ...
            """
            We remove the boundary pixels in a distance of 3 pixels to the boundary 
            because the interfere with the proper calculation of the gradients due to boundary artifacts.
            """
            topo_sorensen_dice_loss = boundary_map_ax.shape[1] + self.SDLoss(boundary_prob_ax[:,:, 3:-3, 3:-3], boundary_map_ax[:,:, 3:-3, 3:-3]).cuda()
...
class SubdivisionLoss(nn.Module):
    ...
    def __init__(self, SD_weight, topo_weight, topo_SD_weight, enable_logging=True, pretraining=False, weight=None, channelwise=True, eps=1e-6):
        ...
        self.SDLoss = SorensenDiceLoss()
        self.TopoLoss = SubdivisionLossFunction.apply
